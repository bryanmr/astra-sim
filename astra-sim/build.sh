...
then
    ...
    if [ "$backend" == "gem5" ]
    then
        git clone https://github.com/georgia-tech-synergy-lab/gem5_astra.git network/gem5_astra && \
            echo "gem5 has been successfully downloaded" || \
            echo "gem5 downloaded did not succeed!"
    elif [ "$backend" == "ns3" ]
    then
        git clone https://github.com/nsnam/ns-3-dev-git.git network/ns3 && \
...
then
    ...
    git clone https://github.com/ARM-software/SCALE-Sim.git compute/SCALE-Sim && \
